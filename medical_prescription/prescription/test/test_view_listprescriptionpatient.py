...
from prescription.views import CreatePrescriptionView, ListPatientPrescription


class TestListPrescription(TestCase):
    def setUp(self):
        ...
        self.patient = Patient.objects.create_user(email='paciente@emp.com',
                                                   password='senha12')

    def teste_list_prescription(self):
        request = self.factory.get('prescription/list_prescription_patient/')
        request.user = self.patient

        response = ListPatientPrescription.as_view()(request)

...
class TestAddPatientForm(TestCase):
    ...
    def test_forms_add_patient_email_is_valid(self):
        context = {'email': self.email_valid}
        form = self.form(data=context)

        self.assertTrue(form.is_valid())

    def test_forms_add_patient_email_is_not_valid_TYPE(self):
        context = {'email': self.email_invalid_TYPE}

        form = self.form(data=context)
        self.assertFalse(form.is_valid())

    def test_forms_add_patient_email_is_not_valid_MAX(self):
        context = {'email': self.email_invalid_MAX}

        form = self.form(data=context)
        self.assertFalse(form.is_valid())

    def test_forms_add_patient_email_is_not_valid_MIN(self):
